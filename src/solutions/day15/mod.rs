...
fn parse_input(input: &str) -> Result<(u64, u64), &'static str> {
    ...
    if lines.len() < 2 {
        return Err("Input must contain at least 2 lines");
    }
    ...
    let a_start = lines[0]
        .split_whitespace()
        .last()
        .ok_or("Invalid format for Generator A line")?
        .parse::<u64>()
        .map_err(|_| "Failed to parse Generator A starting value")?;

    // Parse "Generator B starts with XXX"
    let b_start = lines[1]
        .split_whitespace()
        .last()
        .ok_or("Invalid format for Generator B line")?
        .parse::<u64>()
        .map_err(|_| "Failed to parse Generator B starting value")?;
...
impl Generator {
    ...
    /// Uses bit masking for power-of-2 divisibility checks
    fn next_with_criteria(&mut self, divisible_by: u64) -> u64 {
        debug_assert!(
            divisible_by.is_power_of_two(),
            "divisible_by must be a power of 2"
        );
        let mask = divisible_by - 1;
        loop {
            let val = self.next();
            if val & mask == 0 {
                return val;
...
pub fn solve_part1(input: &str) -> u32 {
    let (a_start, b_start) = parse_input(input).expect("Failed to parse input");
...
mod tests {
    ...
    fn test_part1_example() {
        ...
        let (a_start, b_start) = parse_input(example_input).unwrap();
