...
fn main() -> Result<()> {
    ...
    println!("Submitting answer '{}' for year {}, day {}, part {}...", 
             args.answer, args.year, args.day, args.part);

    submit_answer(&client, &session_cookie, args.year, args.day, args.part, &args.answer)?;

    Ok(())
}

fn submit_answer(client: &Client, session: &str, year: u16, day: u8, part: u8, answer: &str) -> Result<()> {
    ...
    if response.status() != 200 {
        return Err(anyhow!("Failed to submit answer: HTTP {}", response.status()));
    }
